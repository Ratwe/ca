...
def get_value_for_3d_newton(table, x, y, nx, ny):
    ...
    for j in range(len(table)):
        temp = [[i, table[j][i], 1] for i in
                range(len(table[0]))]  # фиксируем одну строку [[0, num, 1 ] ... [i, num, 1 ]]
...
def get_value_for_4d_newton(table, x, y, z, nx, ny, nz):
    ...
    for i in range(len(table)):
        new_table[i] += [get_value_for_3d_newton(table[i], x, y, nx, ny),
                         1]  # фиксируем zi и считаем для 3 мерного 2 переменных
...
def main():
    data = read_table('D:\Code\Github\ca\lab_03\data.txt')
